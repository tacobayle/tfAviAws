...
output "destroy" {
  value = "ssh -o StrictHostKeyChecking=no -i ${var.ssh_key.private} -t ubuntu@${aws_instance.jump.public_ip} 'git clone ${var.ansible["aviPbAbsentUrl"]} --branch ${var.ansible["aviPbAbsentTag"]}; ansible-playbook ansiblePbAviAbsent/local.yml --extra-vars @${var.aws.controller["aviCredsJsonFile"]}' ; sleep 20 ; terraform destroy -auto-approve -var-file=aws.json"
